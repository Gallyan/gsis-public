...
class EditExpense extends Component
{
    ...
    protected function transport_rules()
    {
        return [
    ...
    public function del_reg(int $id)
    {
        ...
        $registrations = $this->expense->registrations;
        if ($id < 1 || $id > count($registrations)) {
            return;
        }
        if (isset($registrations[$id - 1])) {
            unset($registrations[$id - 1]);
        }
    ...
    public function del_transport(int $id)
    {
        ...
        if (isset($transports[$id - 1])) {
            unset($transports[$id - 1]);
        }
