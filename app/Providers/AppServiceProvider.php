...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Validator::replacer(
            'phone', function ($message, $attribute, $rule, $parameters) {
        ...
        Validator::extend(
            'validrole', function ($attribute, $value, $parameters, $validator) {
                return Role::all()->contains('name', $value);
            }
        );
        Validator::replacer(
            'validrole', function ($message, $attribute, $rule, $parameters) {
        ...
        Validator::extend(
            'float', function ($attribute, $value, $parameters, $validator) {
        ...
        Validator::replacer(
            'float', function ($message, $attribute, $rule, $parameters) {
        ...
        Builder::macro(
            'search', function ($field, $string) {
                return $string ? $this->where($field, 'like', '%'.$string.'%') : $this;
            }
        );
        Builder::macro(
            'orSearch', function ($field, $string) {
                return $string ? $this->orWhere($field, 'like', '%'.$string.'%') : $this;
            }
        );
        Builder::macro(
            'searchBefore', function ($field, $date) {
        ...
        Builder::macro(
            'toCsv', function () {
                ...
                $values = $results->map(
                    function ($result) {
                        return implode(
                            ',', collect($result->getAttributes())->map(
                                function ($thing) {
                                    return '"'.$thing.'"';
                                }
                            )->toArray()
