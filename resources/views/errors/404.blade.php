...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    <head>
        ...
        <title>{{ __('404 error') }} - {{ config('app.name', 'Gsis 2') }}</title>
    ...
    <body class="antialiased font-sans h-full">
        ...
        <div class="bg-gray-100 min-h-full px-4 py-16 sm:px-6 sm:py-24 md:grid md:place-items-center lg:px-8">
            <div class="max-w-max mx-auto">
                ...
                <main class="flex">
                    <p class="text-4xl font-extrabold text-indigo-800 sm:text-5xl">404</p>
                    <div class="ml-2 sm:ml-6">
                        <div class="sm:border-l sm:border-gray-200 sm:pl-6">
                            <h1 class="text-4xl font-extrabold text-gray-700 tracking-tight sm:text-5xl">{{ __('Page not found.') }}</h1>
                            <p class="mt-1 text-base text-gray-500">{{ __('Sorry, we couldn’t find the page you’re looking for.') }}</p>
                        </div>
                    </div>
                </main>
                <div class="flex-shrink-0 flex justify-center">
