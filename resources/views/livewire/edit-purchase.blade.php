...
@endpush
        ...
        <div class="mt-6 sm:mt-5">
            ...
            <x-input.group label="Institution" for="institution_id" :error="$errors->first('purchase.institution_id')" required>
                ...
                @if ( $purchase->institution?->to && $purchase->institution->to->lt(Illuminate\Support\Carbon::today()) )
                <p class="mt-4 text-red-500 text-sm leading-5">
                    <x-icon.warning class="mr-2 flex-shrink-0 h-6 w-6" />
                    {{ __('Institution was available until :date', [
                        'date' => $purchase->institution->to->format('d/m/Y'),
                        ]) }}
                </p>
                @endif
            </x-input.group>

            <x-input.group label="WP" for="wp" :error="$errors->first('purchase.wp')" :required="$showWP">
                @if ($showWP)
                    <x-input.number wire:model="purchase.wp" id="wp" min="1" :disabled="$disabled" :print="$purchase->wp" />
                @else
                ...
                <x-table>
                    <x-slot name="head">
                        <x-table.heading colspan="2">
                            ...
                            @if(!$disabled)
                            <span class="inline-flex text-sm leading-5 pr-4 print:hidden">
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row>
                            ...
                            <x-table.cell>
                                ...
                                <x-icon.trash class="ml-5 mr-1 text-gray-500 cursor-pointer print:hidden" wire:click="del_list({{ $loop->index }})"/>
                                <div>
            ...
            <x-input.group label="Send your documents" for="uploads" :error="$upload_errors->all()" >
                ...
                @if (count($purchase->documents)>1)
                <p class="text-sm font-medium text-gray-700">
                    <a href="{{ route( 'zip-purchase', $purchase ) }}" class="hover:underline">
