                        ...
                        <x-input.select wire:model="filters.institution" id="filter-institution" class="w-full" multiple>
                            @php
                                if( auth()->user()->can('manage-users') ) {
                                    $user_inst = \App\models\Purchase::select('institution_id')->distinct()->get()->pluck('institution_id');
                                } else {
                                    $user_inst = \App\models\Purchase::where('user_id','=',auth()->id())->select('institution_id')->distinct()->get()->pluck('institution_id');
                                }
                            @endphp
        ...
        <div class="flex-col space-y-4">
            <x-table>
                ...
                <x-slot name="body">
                    ...
                    <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $purchase->id }}" wire:click="edit({{ $purchase->id }})" class="cursor-pointer hover:bg-gray-100 {{ $loop->iteration % 2 == 0 ? 'bg-gray-50' : '' }}">
                        ...
                        <x-table.cell class="whitespace-nowrap">
                            {!! nl2br(e($purchase->managers->pluck('name')->unique()->implode(PHP_EOL))) !!}
                        </x-table.cell>
